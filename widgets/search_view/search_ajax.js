...
function process_search()
{
	if(xmlHttp.readyState==0 || xmlHttp.readyState==4)
	{
		...
		xmlHttp.open("GET", "./widgets/search_view/search_xml.php?search_parameter=" + search_parameter + "&search_type=" + search_type, true);
		...
		if(xmlHttp.status==200)
		{
			...
			root = xmlResponse.documentElement;
				...
				for(var i=0; i< first_name.length; i++)
				{
					...
					woLink = document.createElement('a');
					
					
					a = (wo_id.item(i).firstChild) ? document.createTextNode(wo_id.item(i).firstChild.data) : document.createTextNode("N/A");
					...
					woLink.appendChild(a);
					woLink.setAttribute('href', 'widgets/wo_create_edit/edit_view.php?id=' + a.data);
					woLink.setAttribute('target', 'blank');
					
					woId.appendChild(woLink);   //add the text to the li's
